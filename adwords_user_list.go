...
type UserListOperations map[string][]MutateMembersOperand
...
type MutateMembersOperand struct {
	UserListId string `xml:"userListId"`
...
func (s *AdwordsUserListService) Mutate(adwordsUserListOperations UserListOperations) (adwordsUserLists []UserList, err error) {
	type MutateMembersOperation struct {
		...
		MutateMembersOperand MutateMembersOperand `xml:"operand"`
	}
	operations := []MutateMembersOperation{}
	for action, campaigns := range adwordsUserListOperations {
		for _, campaign := range campaigns {
			operations = append(operations,
				MutateMembersOperation{
					Action:   action,
					MutateMembersOperand: campaign,
				},
			)
		}
	}
	mutation := struct {
		XMLName xml.Name
		Ops     []MutateMembersOperation `xml:"operations"`
	}{
		XMLName: xml.Name{
			Space: "https://adwords.google.com/api/adwords/rm/v201609",
			Local: "mutateMembers",
		},
		Ops: operations}
		fmt.Printf("%+v\n",mutation)
	...
	mutateResp := struct {
		MutateMembersOperands []UserList `xml:"rval>value"`
	}{}
	...
	return mutateResp.MutateMembersOperands, err
}
